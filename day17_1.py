...
stack_size = 1 + (2 * runs)
board = np.zeros([stack_size, layer_size, layer_size], dtype='int') 
mid = (int)(stack_size / 2)
for x in range(initial_size):
    for y in range(initial_size):
        board[mid][y+runs][x+runs] = initial[y][x]
#print(f"initial:\n{board}")
    ...
    for z in range(stack_size):
            for x in range(layer_size):
                for y in range(layer_size):
                        ...
                        for yp in [y - 1, y, y + 1]:
                            for zp in [z - 1, z, z + 1]:
                                    ...
                                    if (xp < 0) or (yp < 0) or (zp < 0):
                                        continue
                                    if (xp >= layer_size) or (yp >= layer_size) or (zp >= stack_size):
                                        continue
                                    c = board[zp][xp][yp]
                                    if c == 1:
                                        neighbors += 1
                    c = board[z][x][y]
                    if c == 1:
                        new_board[z][x][y] = 1 if (neighbors == 2) or (neighbors == 3) else 0
                    if c == 0:
                        new_board[z][x][y] = 1 if (neighbors == 3) else 0
    current = 1 - current
